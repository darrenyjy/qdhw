...
public class LineStorage
{
	...
	public void setChar(char c, int position, int word, int line)
	{
		...
		String current_word = (String) current_line.getWord(word);
		...
		// make new string representation of the word
		current_word = new String(chars);

		// replace the old word with the new one
		current_line.setWord(current_word, word);
	}
	...
	{
		return lines_.get(line).getWord(word).charAt(position);
	}
	...
	{
		return lines_.get(line).getWord(word).length();
	}
