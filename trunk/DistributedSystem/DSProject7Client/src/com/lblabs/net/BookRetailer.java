...
System.out.println(threadID + ": (online request result): " + response);
			try
			{
				Thread.sleep(1000);
			}
...
System.out.println(threadID + ": signInServer response = " + response);
	...
	{
		request = threadID + "|signUp|" + newAccountName + "#" + newAccountPassword + "#" + newCreditCardNumber + "#|";
		response = this.sendRequest(request);
	...
	{
		request = threadID + "|signIn|" + inputAccountName + "#" + inputAccountPassword + "#|";
		response = this.sendRequest(request);
		Hashtable responseHash = tools.convertStringToHash(response);
		return tools.convertStringToBoolean((String)responseHash.get("0"));
	}
...
System.out.println("threadID (BookRetailer Proxy) = " + threadID);
		response = this.sendRequest(request);
		Hashtable responseHash = tools.convertStringToHash(response);
		if (response != null)
		{
	...
	{
		request = threadID + "|putIntoShoppingCart|" + bookName + "#" + tools.convertIntToString(quantity) + "#|";
		response = this.sendRequest(request);
		Hashtable responseHash = tools.convertStringToHash(response);
		return tools.convertStringToBoolean((String)responseHash.get("0"));
	}
	...
	{
		request = threadID + "|moveOutShoppingCart|" + bookName + "#" + tools.convertIntToString(quantity) + "#|";
		response = this.sendRequest(request);
		Hashtable responseHash = tools.convertStringToHash(response);
		return tools.convertStringToBoolean((String)responseHash.get("0"));
	}
	...
	{
		request = threadID + "|buyBooks|" + inputCreditCardNumber + "#|";
		response = this.sendRequest(request);
		Hashtable responseHash = tools.convertStringToHash(response);
		return responseHash;
	}
