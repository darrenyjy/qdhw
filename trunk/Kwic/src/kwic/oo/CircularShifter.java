...
/**
 * An object of the CircularShifter class produces and holds all circular shifts
 * of a set of lines. A set of lines may be passed to a CircularShifter object,
 * which produces all circular shifts and stores them internally. In principle,
 * the CircularShifter class provides a similar interface as the LineStorage
 * class, thus allowing to manipulate the lines that it holds. However, in the
 * case of the CircularShifter class the lines are actually circular shifts of a
 * particular set of original lines. Also, the CircularShifter class does not
 * provide interface for updating of characters, words, and lines that it holds,
 * but just an interface for reading characters, words, and lines.
 * 
 * @author dhelic
 * @version $Id$
 */
...
public class CircularShifter
{
	...
	// ----------------------------------------------------------------------
	/**
	 * Produces all circular shifts of lines in a given set. Circular shifts are
	 * stored internally and can be queried by means of other methods. Note,
		...
		for (int i = 0; i < lines.getLineCount(); i++)
		{
			...
			for (int j = 0; j < line.length; j++)
			{
				...
				for (int k = j; k < (line.length + j); k++)
					...
					shifts_.addWord(line[k % line.length], shifts_
							.getLineCount() - 1);
	...
	// ----------------------------------------------------------------------
	/**
	 * Gets the line from the specified position. String representing the line
	 * is returned.
	 * 
	 * @param line
	 *            line index
